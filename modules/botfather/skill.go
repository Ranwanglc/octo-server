...
`+"```"+`bash
npx -y create-openclaw-octo bind --bot-token <bot_token> --api-url <apiUrl> --account-id <robot_id> --agent <agent_id>
`+"```"+`

**All agents — one-click setup (get key from BotFather /quickstart):**
`+"```"+`bash
npx -y create-openclaw-octo quickstart --api-key <api_key> --api-url <apiUrl>
`+"```"+`
...
Source & docs: https://www.npmjs.com/package/create-openclaw-octo
...
2. Run `+"`"+`npx -y create-openclaw-octo quickstart --api-key <key> --api-url <apiUrl>`+"`"+`
